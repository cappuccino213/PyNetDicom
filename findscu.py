...
# 导入的common模块是修改过的，优化了输出目录，增加了查询层次，不能用默认的必须用修改过的common
...
from configuration import qr_level
from pynetdicom.sop_class import _QR_CLASSES

# 设置相关pynetdicom uid
ModalityWorklistInformationFind = _QR_CLASSES.get('ModalityWorklistInformationFind')
PatientRootQueryRetrieveInformationModelFind = _QR_CLASSES.get('PatientRootQueryRetrieveInformationModelFind')
StudyRootQueryRetrieveInformationModelFind = _QR_CLASSES.get('StudyRootQueryRetrieveInformationModelFind')
PatientStudyOnlyQueryRetrieveInformationModelFind = _QR_CLASSES.get('PatientStudyOnlyQueryRetrieveInformationModelFind')
...
def main(args=None):
    ...
    if assoc.is_established:
        ...
        print('返回{0}数量：{1}'.format(qr_level, count))
