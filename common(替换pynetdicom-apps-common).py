...
def handle_store(event, args, app_logger):
    ...
    try:
        ...
        # 获取检查类型前缀,如果获取不到,可以从venv\Lib\site-packages\pynetdicom\sop_class.py的_STORAGE_CLASSES获得对照，在SOP_CLASS_PREFIXES补上去
    ...
    if args.output_directory is not None:
        ...
        store_dir = '{0!s}/{1!s}'.format(today, study_instance) # 相对目录 (配置路径+)日期+检查实例号
        out_put_dir = os.path.join(args.output_directory, store_dir) # 实际输出路径
